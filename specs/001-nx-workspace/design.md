...
- **Scripts:** `package.json` has `"format": "nx format:write --all"` and `"format:check": "nx format:check --all"`, so people, agents and a later CI job use one name with `--all` built in. The root `package.json` gets no `nx` field: with one, Nx turns the root into an `inkhr` project, and `nx show projects` no longer lists none. Scripts alone do not.
...
| `package.json`                                                 | `name: "inkhr"`, `private: true`, `engines.node: ">=22.22.1"`, `workspaces: ["packages/*"]`, the `format` and `format:check` scripts from "Formatter", the `prepare` script and `lint-staged` config from "Automatic formatting", no `nx` field, exact versions for `nx`, `@nx/js`, `@nx/eslint`, `@nx/eslint-plugin` (23.2.1), `eslint`, `typescript` 6.0.3, `@changesets/cli` 3.0.3, `prettier` 3.9.7 and `lint-staged` 17.5.1.       |
