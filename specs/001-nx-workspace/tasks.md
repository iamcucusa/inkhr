...
Work on the branch `001-nx-workspace`, named after this spec folder as `docs/commit-guide.md` describes. One commit per task, following that guide; stop after each task and propose the commit message for approval. Open one pull request at the end and merge it with a rebase merge.
