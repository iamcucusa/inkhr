...
2. Check: `npm test` runs and reports no test files.

Commit: `build(repo): add vitest for repository tools`

## 2. Write the checkers

Depends on task 1.

1. Write `rules.mjs`, `check-message.mjs`, `check-branch.mjs` and `check-range.mjs` from "The rules" in `design.md`, with the disclosure part left out.
2. Write their tests: one passing and one failing case per rule, including the pass-through cases.
3. Check: `npm test` passes. `check-message.mjs` exits 1 on a message with a bad type, an over-long header, an uppercase letter after the colon, a trailing period, a three-line body or a trailer, and 0 on a good one. `check-branch.mjs` exits 1 on `claude/some-work` and `Feature_X`, and 0 on `002-commit-rules` and `main`.

Commit: `build(repo): add commit message and branch checkers`

## 3. Add the disclosure check
...
## 6. Write the guide and the agent pointer

Depends on task 2, because the guide states the rules the checkers enforce.
...
2. Check, after the pull request in task 9 exists: the job passes on this branch, and a scratch commit that breaks a rule makes it fail with the rule named and no term in the log. Remove the scratch commit afterwards.
